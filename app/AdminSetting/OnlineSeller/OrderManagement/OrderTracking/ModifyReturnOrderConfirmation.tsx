...
interface ModifyProps {
  ...
  setVarientID: (data: string) => void;
  setProductID: (data: string) => void;
  setSellerID: (data: string) => void;
  setEmail: (data: string) => void;
  setQty: (data: string) => void;
  setRate: (data: string) => void;
  setVaraientValue: (data: string) => void;
}
export default function ModifyReturnOrderConfirmation({
  StoreID,
      ...
      const filteredData = data.order
        .map((item) => ({
          ...
          bags: item.bags.filter(
            (bags) =>
              bags.orderType === "Return" &&
              (bags.status === "packed" || bags.status === "shipped"),
          ),
        }))
                                ...
                                <div className="w-full flex items-center justify-between">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => {
                                        {
                                          ...
                                          if (bag.status === "shipped") {
                                            ...
                                            setBagNo(bag.bagNo);
                                            setVarientID(
                                              bag.product[0].varientID,
                                            );
                                            setProductID(
                                              bag.product[0].productID,
                                            );
                                            setSellerID(bag.sellerID);
                                            setEmail(bag.email);
                                            setQty(String(bag.product[0].qty));
                                            setRate(
                                              String(bag.product[0].rate),
                                            );
                                            setVaraientValue(
                                              bag.product[0].varintValue
                                                .map((item) => item.value)
                                                .join("-"),
                                            );
                                      ...
                                      title={`${bag.status === "shipped" ? "Return Order" : "Approve Order"}`}
                                    >
                                      {bag.status === "shipped" ? (
                                        <>
                                          <Repeat className="mx-auto h-5 w-5" />
                                        </>
                                      ) : (
